...
```{r}
spanish_books <- books_tidy %>% 
  filter(language_code == "spa")
```


```{r}
spanish_observed_stat <- spanish_books %>% 
  specify(response = average_rating) %>% 
  calculate(stat = "mean")
```
...
```{r}
spanish_null_distribution <- spanish_books %>% 
  specify(response = average_rating) %>% 
...
```{r}
spanish_null_distribution %>% 
  visualise() +
  shade_p_value(obs_stat = spanish_observed_stat, direction = "left")
```


```{r}
spanish_p_value <- spanish_null_distribution %>% 
  get_p_value(obs_stat = spanish_observed_stat, direction = "left")
